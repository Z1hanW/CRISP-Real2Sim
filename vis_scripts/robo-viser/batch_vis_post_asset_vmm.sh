...
declare -a TASKS=()
for exp_dir in "$DATA_ROOT"/post_asset_videomimic_*; do
  [[ -d "$exp_dir" ]] || continue
  ...
  for seq_dir in "$exp_dir"/*; do
    ...
    TASKS+=("${parent_name}_${seq_name}_videomimic")
  done
done

if [[ ${#TASKS[@]} -eq 0 ]]; then
  echo "No sequences found under $DATA_ROOT/post_asset_videomimic_*" >&2
  exit 1
fi
