...
from viser._messages import SetCameraLookAtMessage, SetCameraPositionMessage
...
def render_robot_with_external_camera(
    ...
    for idx, t in enumerate(render_frames):
        ...
        def _apply_camera(active_client):
            with active_client.atomic():
                active_client.camera.position = pos
                active_client.camera.wxyz = quat
                active_client.camera.fov = float(fov)

        _apply_camera(client)
        print(pos, quat, fov)
        time.sleep(dt)
        rgb = None
        for attempt in range(max_render_attempts):
            try:
                rgb = client.get_render(
                    height=int(cam_data["H"]),
                    width=int(cam_data["W"]),
                    timeout=render_timeout,
                )
                break
            except TimeoutError:
                print(
                    f"[Renderer] Frame {t}: render timeout ({render_timeout:.1f}s). "
                    f"Attempt {attempt + 1}/{max_render_attempts}; trying to reconnect."
                )
                _close_launched(launched)
                try:
                    client, launched = _get_client_or_launch(viser_helper.server)
                except TimeoutError as exc:
                    print(f"[Renderer] Unable to reconnect: {exc}")
                    break
                robot.update(robot_pos[t], robot_rot[t])
                _apply_camera(client)
                time.sleep(1.5)
...
def main():
    ...
    ap.add_argument(
        ...
        dest="save_images",
        action="store_true",
        help="Enable saving every rendered frame as PNG",
    )
    ap.add_argument(
        "--no_save_images",
        dest="save_images",
        action="store_false",
        help="Disable saving individual frame PNGs",
    )
    ap.add_argument(
        "--save_video",
        dest="save_video",
        action="store_true",
        help="Save the rendered MP4 sequence",
    )
    ap.add_argument(
        "--no_save_video",
        dest="save_video",
        action="store_false",
        help="Skip writing the rendered MP4 sequence",
    )
    ...
    ap.add_argument(
        "--run_name",
        type=str,
        help="Name used when saving the offline .viser recording (defaults to DATE_SCENE_METHOD)",
    )
    ap.add_argument(
        "--viser_output_dir",
        type=str,
        default="./viser_output",
        help="Directory to store serialized .viser recordings",
    )
    ap.add_argument(
        "--save_mode",
        action="store_true",
        help="Stop the Viser server as soon as rendering completes",
    )

    ap.set_defaults(save_images=True, save_video=True)

    args = ap.parse_args()

    args.output_dir = os.path.join(args.output_dir, f'{args.parent_name}_{args.scene}')
    os.makedirs(args.output_dir, exist_ok=True)
    recording_stem = args.run_name or f"{args.parent_name}_{args.scene}_{args.method}"
    ...
    if info_path.exists():
        ...
        asset_xml_rel = None
    dt = 0.5 # *= 3.0  # allow extra settle time between rendered frames
    ...
    try:
        ...
        follow_camera_track = camera_handler is not None and len(camera_handler.images) > 0
        camera_forward = np.array([0.0, 0.0, 1.0], dtype=np.float32) if follow_camera_track else None
        if follow_camera_track:
            frame_idx = 1 if len(camera_handler.images) > 1 else 0
            cam_data = camera_handler.get_camera_at_frame(frame_idx)
            T_c2w = np.asarray(cam_data["T_c2w"], dtype=np.float32)
            cam_pos = T_c2w[:3, 3].astype(float)
            forward = (T_c2w[:3, :3] @ camera_forward).astype(float)
            look_at = cam_pos + forward
            serializer._insert_message(
                SetCameraPositionMessage(position=tuple(float(x) for x in cam_pos))
            )
            serializer._insert_message(
                SetCameraLookAtMessage(look_at=tuple(float(x) for x in look_at))
            )
            print(f"[Serializer] Initialized camera from frame {frame_idx} of external camera data.")

        timesteps = len(pos)
        print(timesteps)
        for t in range(timesteps):
            if follow_camera_track:
                # Mirror the dataset camera path inside the serialized recording.
                cam_data = camera_handler.get_camera_at_frame(t)
                T_c2w = np.asarray(cam_data["T_c2w"], dtype=np.float32)
                cam_pos = T_c2w[:3, 3].astype(float)
                forward = (T_c2w[:3, :3] @ camera_forward).astype(float)
        ...
        viser_output_dir = Path(args.viser_output_dir)
        viser_output_dir.mkdir(parents=True, exist_ok=True)
        viser_path = viser_output_dir / f"{recording_stem}.viser"
        viser_path.write_bytes(data)
        print(f"[Serializer] Saved viser recording to {viser_path}")

        if args.save_mode:
            print("[Visualizer] Save mode enabled; stopping server now.")
