...
CAMERA_DICT_EXPORT_DIR = Path("/data3/zihanwa3/_Robotics/__release/results/output/camera_dict")
...
def _export_camera_dict(camera_dict: dict, run_name: str):
    """Persist the merged camera dictionary for downstream inspection."""
    if not camera_dict:
        return

    export_dir = CAMERA_DICT_EXPORT_DIR
    export_dir.mkdir(parents=True, exist_ok=True)

    safe_name = (run_name or "camera_dict").replace("/", "_")
    out_path = export_dir / f"{safe_name}.npz"
    try:
        np.savez_compressed(out_path, **camera_dict)
        print(f"[Camera] Saved merged camera_dict to {out_path}")
    except Exception as exc:  # best-effort export; never fail render
        print(f"[Camera] Failed to export camera_dict to {out_path}: {exc}")
...
def main():
    ...
    try:
        ...
        export_name = args.run_name or recording_stem
        _export_camera_dict(camera_dict, export_name)
        camera_handler = ExternalCameraHandler(
