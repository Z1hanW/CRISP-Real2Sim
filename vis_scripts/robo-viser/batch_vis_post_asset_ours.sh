...
DATA_ROOT="/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour"
...
declare -a TASKS=()
for exp_dir in "$DATA_ROOT"/post_asset_ours_*; do
  [[ -d "$exp_dir" ]] || continue
...
if [[ ${#TASKS[@]} -eq 0 ]]; then
  echo "No sequences found under $DATA_ROOT/post_asset_ours_*" >&2
  exit 1
fi
